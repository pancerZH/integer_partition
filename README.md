...
其中，五边形数定理在组合数学课上并没有介绍，所以我只实现了前三个方法。
...
## 结果分析
为了避免递归方法运行时间太长，阻塞程序，所以我规定，当x > 80时，不再执行递归方法。当x大于一定数值时，母函数方法也会变慢，所以当x > 2000时，不再执行母函数方法。三种方法的运行效果如下：
...
可以看到，随着x的增大，p(x)增长得越来越快。我在电影《知无涯者》中看到麦克马洪手算了前200个整数的拆分数，在观影时我并没有在意这个细节，但是知道我今天真的算出了p(200) = 3972999029388，我才意识到每个时代的数学家都有着自己的伟大之处。  
